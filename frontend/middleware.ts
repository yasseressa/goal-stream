...
export function middleware(request: NextRequest) {
  ...
  if (!firstSegment || !locales.includes(firstSegment as (typeof locales)[number])) {
    ...
    response.cookies.set("kinglive-locale", preferredLocale);
    return response;
  }

  const response = NextResponse.next();
  response.cookies.set("kinglive-locale", firstSegment);
  return response;
}
...
function getPreferredLocale(request: NextRequest) {
  const cookieLocale = request.cookies.get("kinglive-locale")?.value;
